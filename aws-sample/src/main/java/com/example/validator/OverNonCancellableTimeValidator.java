...
@Component
public class OverNonCancellableTimeValidator implements ConstraintValidator<OverNonCancellableTime, ReservationForm> {

	@Autowired
	private PermissionHourRepository permissionHourRepository;

    public OverNonCancellableTimeValidator() {
    	this.permissionHourRepository = null;
    }

    @Autowired
    public OverNonCancellableTimeValidator(PermissionHourRepository permissionHourRepository) {
    ...
    public boolean isValid(ReservationForm value, ConstraintValidatorContext context) {
        ...
        || value.getMember() == null || value.getFacility() == null || value.getRday() == null) {
            return true; //予約時間が未入力の時はこのエラーは表示しない。NotNullの単体エラー表示に任せる //また、新規のときは不問
          }
